...
object KafkaDirectWordCount {
  def main(args: Array[String]): Unit = {
    ...
    //指定kafka的broker地址(SparkStreaming的task直连到kafka分区上，用更加底层的api消费，效率更高)
    val brokerList = "hadoop1:9092,hadoop2:9092,hadoop3:9092"
    //指定zk的地址，后期更新消费的偏移量时使用(以后可以使用redis,Mysql来记录偏移量)
    val zkQuorum = "hadoop1:2181"
    val grooupId = "g001"
    //指定消费的topic名字
    val topic = "wordcount"
    //创建stream时使用的topic集合，SparkStreaming可同时消费多个topic
    val topics:Set[String] = Set(topic)
    //创建一个ZKgroupTopicDirs对象，其实是指定往zk中写入数据的目录，用于保存偏移量
    val topicDirs = new ZKgroupTopicDirs(grooupId, topics)
    //获取zookeeper中的路径 "/g001/offsets/wordcout"
    val zkTopicPath = s"${topicDirs.consumerOffsetDir}"

    //准备kafka的参数
    val kafkaParams = Map(
      "matadata.broker.list" ->brokerList,
      "group.id" -> grooupId,
      //重头开始读数据
      "auto.offset.reset" -> kafka.api.OffsetRequest.SmallestTimeString
    )
    //是zookeeper的客户端，可以从zk读取偏移量数据，并更新偏移量
    val zkClient = new ZKClient(zkQuorum)
    val children: util.List[String] = zkClient.listServices(zkTopicPath)
    ...
    if (children > 0) {
      for (i <- 0 until children.size()) {
        ...
        val partitionOffset = zkClient.getServiceData(s"$zkTopicPath/${i}")
        // wordcount/0
